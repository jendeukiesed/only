import { requireBuyer } from "@/lib/auth/rbac";
import { db } from "@/lib/db/prisma";
import { RewardCatalog } from "@/features/rewards/components/reward-catalog";
import { EmptyState } from "@/components/shared/empty-states/empty-state";
import { Gift } from "lucide-react";

export const metadata = { title: "Rewards" };

export default async function BuyerRewardsPage() {
  const user = await requireBuyer();

  const [pointsBalance, allItems] = await Promise.all([
    db.user.findUnique({ where: { id: user.id }, select: { pointsBalance: true } }).then((u) => u?.pointsBalance ?? 0),
    db.rewardItem.findMany({ where: { isActive: true }, orderBy: { pointsCost: "asc" } }),
  ]);

  // Buyers aren't sellers, so seller-only categories (feature boosts,
  // commission discounts) are filtered out here rather than shown
  // disabled — the same catalog data backs both /buyer/dashboard/rewards
  // and /seller/dashboard/rewards, each rendering only what applies.
  const items = allItems.filter((i) => i.category === "PROFILE_BADGE");

  return (
    <div className="mx-auto max-w-6xl space-y-6">
      <div>
        <h1 className="font-display text-2xl font-semibold">Rewards catalog</h1>
        <p className="mt-1 text-sm text-muted-foreground">Redeem points for cosmetic profile rewards.</p>
      </div>

      {items.length === 0 ? (
        <EmptyState icon={Gift} title="No rewards available right now" description="Check back soon." />
      ) : (
        <RewardCatalog items={items} pointsBalance={pointsBalance} />
      )}
    </div>
  );
}
